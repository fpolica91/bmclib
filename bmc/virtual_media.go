...
type VirtualMediaSetter interface {
	SetVirtualMedia(ctx context.Context, kind string, mediaURL string, username string, password string) (ok bool, err error)
}
...
func setVirtualMedia(ctx context.Context, kind string, mediaURL string, b []virtualMediaProviders, username string, password string) (ok bool, metadata Metadata, err error) {
	...
	for _, elem := range b {
		...
		default:
			...
			ok, setErr := elem.virtualMediaSetter.SetVirtualMedia(ctx, kind, mediaURL, username, password)
			if setErr != nil {
...
func SetVirtualMediaFromInterfaces(ctx context.Context,
	kind string,
	mediaURL string,
	username string,
	password string,
	generic []interface{}) (ok bool, metadata Metadata, err error) {
